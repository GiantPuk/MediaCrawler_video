...
async def update_zhihu_content(content_item: ZhihuContent):
    ...
    utils.logger.info(
        ...
        f"zhihu content id:{local_db_item.get('content_id')}, type:{local_db_item.get('content_type')}, "
        f"title:{local_db_item.get('title') or local_db_item.get('desc')}, "
        f"voteup:{local_db_item.get('voteup_count')}, comments:{local_db_item.get('comment_count')}"
    )

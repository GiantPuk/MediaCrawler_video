...
任务状态会记录候选获取、下载、上传、转录和模型分析的阶段结果；平台登录态、视频直链和模型调用异常都会保留在日志与结果文件中，便于复现和排查。
...
其他平台目前更稳定的方式是输入主页链接或平台 creator ID。
...
热搜词和话题类入口主要用于二次关键词检索；视频候选进入下载与总结流程。
...
整体汇总会尝试输出共同主题、各自内容梗概、摘要以及 Mermaid mindmap。若聚合模型没有返回可用文本，会根据单视频摘要生成本地汇总，并在结果中保留对应状态。
...
| 平台 | 搜索候选 | 作者视频 | 榜单/热榜 | 下载与总结 | 注意事项 |
| --- | --- | --- | --- | --- | --- |
| B 站 | 支持直连 WBI 搜索和 MediaCrawler 路径 | 支持 UID 和作者候选消歧 | 支持热门、分区、每周榜单和热搜词 | 支持 playurl 解析、过期重解析、下载后总结 | Cookie 对部分榜单和登录态接口有帮助。 |
| 抖音 | 依赖 Cookie/CDP 或原项目签名逻辑 | 推荐主页链接或 `sec_user_id` | 热搜词可用于二次检索 | 有真实直链时可下载并总结 | 登录态和平台风控会影响稳定性。 |
| 快手 | 依赖 Cookie 与签名接口 | 推荐主页链接或 creator ID | brilliant 热榜候选 | 有真实直链时可下载并总结 | Cookie 失效会直接影响搜索和详情请求。 |
| 小红书 | 可返回视频候选 | 推荐主页链接或 creator ID | 以关键词和作者视频任务为主 | 获取到视频文件后可总结 | 建议低频、低并发并复用真实浏览器登录态。 |
| 微博 | 保留原项目视频搜索路径 | 推荐主页链接或 UID | 热搜词可用于二次检索 | 有真实直链时可下载并总结 | 搜索接口返回的播放、互动字段可能不完整。 |
...
| B 站关键词搜索，走浏览器/MediaCrawler 路径 | 10-30s |
...
8. 登录态异常、下载异常和模型异常都会写入任务日志，便于继续排查。
...
| 视频抓取/下载 | B站、微博、抖音、快手在候选含真实直链时可下载；小红书走详情链路或本地视频后进入总结。 |
| 今天排行榜前 5 | B站可直接返回视频榜单；快手可返回热榜候选；抖音/微博热搜词可继续二次检索视频。 |
...
整体来看，本项目已经完成可演示、可测试、可迭代的版本。B 站链路最完整；抖音、快手、微博、小红书已经覆盖搜索候选、下载入口和视频总结主流程，后续稳定性主要取决于登录态、平台接口变化和视频直链可用性。
